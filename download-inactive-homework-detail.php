...
/* vars for export */
$csv_filename = 'homework_data_'.date('Y-m-d').'.csv';
...
if (!isset($_GET['stoc'])) {
	...
	$c=1;
	$res = mysqli_query($conn,"select * from homework where status='inactive' and update_date>='$cdate1' and update_date<='$cdate2'");
	...
	fputcsv($fp, array('#','Title', 'Type','Institute/Student', 'Program','Insert By','Inserted Name','Insert Date','Txn Date'));
	$c=1;
	$res = mysqli_query($conn,"SELECT * from tbl_stoc_upload where status='inactive' AND type = 'Homework' and update_date>='$cdate1' and update_date<='$cdate2'");
	if(mysqli_num_rows($res)>0)
	{    
		while($getList = mysqli_fetch_assoc($res))
		{ 
