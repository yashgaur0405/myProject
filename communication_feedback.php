...
if (isset($_POST['submit_feedback'])) {
 ...
 $insert_feedback = mysqli_query($conn, "INSERT INTO communication_feedback(type, type_id, message, created_at) VALUES('$type', '$type_id', '$feedback', '$today') ");
 if ($insert_feedback) {
...
if (isset($_POST['submit_reply'])) {
  ...
  $insert_reply = mysqli_query($conn, "INSERT INTO feedback_reply(feedback_id, type, type_id, reply_mess, created_at) VALUES('$feedback_id', '".$role['title']."', '".$_SESSION['email']."', '$reply_content', '$today')");
  if ($insert_reply) {
              ...
              <div class="card-body">
                ...
                if ($role['title'] == 'Admin') { ?>
                  <div class="container">
                    ...
                    $get_feedback = mysqli_query($conn, "SELECT * FROM communication_feedback WHERE created_at >= '$date_from' AND created_at <= '$date_to' ");
                    if(mysqli_num_rows($get_feedback) > 0){
                      while ($listFeedback = mysqli_fetch_assoc($get_feedback)) { 
                        ...
                        $get_replies = mysqli_query($conn, "SELECT * FROM feedback_reply WHERE feedback_id = '".$listFeedback['id']."' ORDER BY created_at DESC ");
                    ...
                    <?php }elseif($role['title'] == 'Staff'){ ?>
                      <div class="container">
                        ...
                        $count=1;
                        $login = $_SESSION['email'];
                        $get_feedback = mysqli_query($conn, "SELECT * FROM communication_feedback WHERE created_at >= '$date_from' AND created_at <= '$date_to' AND type_id = '$login' OR type_id IN (SELECT enrollment_no FROM tbl_student_detail WHERE institute IN (SELECT associate_id FROM tbl_associate_detail WHERE multi_edu_email = '$login') ) ");
                        if(mysqli_num_rows($get_feedback) > 0){
                          while ($listFeedback = mysqli_fetch_assoc($get_feedback)) { 
                            $get_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE email = '".$listFeedback['type_id']."' "));
                            $get_replies = mysqli_query($conn, "SELECT * FROM feedback_reply WHERE feedback_id = '".$listFeedback['id']."' ORDER BY created_at DESC ");
                        ...
                        <?php }elseif($role['title'] == 'Associate'){ ?>
                          <div class="container">
                            ...
                            $count=1;
                            $login = $_SESSION['email'];
                            $get_feedback = mysqli_query($conn, "SELECT * FROM communication_feedback WHERE type_id = '$login' AND created_at >= '$date_from' AND created_at <= '$date_to' ");
                            if(mysqli_num_rows($get_feedback) > 0){
                              while ($listFeedback = mysqli_fetch_assoc($get_feedback)) { 
                                $get_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE email = '".$listFeedback['type_id']."' "));
                                $get_replies = mysqli_query($conn, "SELECT * FROM feedback_reply WHERE feedback_id = '".$listFeedback['id']."' ORDER BY created_at DESC ");
                                ...
                                <div class="collapse" id="collapseExample<?php echo $count; ?>">
                                  <div class="card container card-body">
                                    <?php while ($getAllReplies = mysqli_fetch_assoc($get_replies)) { 
                                      $get_replies_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE email = '".$getAllReplies['type_id']."'"));
                                      ?>
                                      <h6><span style="float: left; font-weight: bold; font-size: 16px; "><?php if($getAllReplies['type'] == 'Admin'){ echo 'Admin:&nbsp;&nbsp;' ;}elseif($_SESSION['email'] == $getAllReplies['type_id']){ echo 'You:&nbsp;'; }else{ echo $get_replies_name['first_name'].' '.$get_replies_name['last_name'].'('.$getAllReplies['type'].'): ';} ?></span><span><?php echo date_format(date_create($getAllReplies['created_at'], 'd-M-Y')); ?></span><span style="color: lightgrey; float: right"><?php echo $getAllReplies['created_at'] ?></span></h6>
                                    <?php } ?>
                                    <div><hr>
                                      <form method="POST" action="" style="float: right;">  
                                        <input type="hidden" value="<?php echo $listFeedback['id']; ?>" name="reply_feedback_id">
