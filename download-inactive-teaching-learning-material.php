...
/* vars for export */
$csv_filename = 'teaching_learning_material_data_'.date('Y-m-d').'.csv';
...
if (!isset($_GET['stoc'])) {
	...
	$c=1;
	$res = mysqli_query($conn,"select * from teaching_learning_material where status='inactive' and update_date>='$cdate1' and update_date<='$cdate2'");
	...
	{    
		...
		{ 
			...
			fputcsv($fp, array($c++,$getList['title'],$getList['type'],$stu_ins_name,$getList['program_name'],$getList['inserted_by'],$get_name['first_name'].' '.$get_name['last_name'],$getList['inserted_date'],$getList['update_date']));
		}
	}
}elseif (isset($_GET['stoc'])) {
	fputcsv($fp, array('#','Title', 'Type','Institute/Student', 'Program','Insert By','Inserted Name','Insert Date','Txn Date'));
	$c=1;
	$res = mysqli_query($conn,"SELECT * from tbl_stoc_upload where status='inactive' AND type = 'Teaching Learning' and update_date>='$cdate1' and update_date<='$cdate2'");
	if(mysqli_num_rows($res)>0)
	{    
		while($getList = mysqli_fetch_assoc($res))
		{ 
